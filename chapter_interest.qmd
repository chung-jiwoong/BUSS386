...
- Treasury Rates
  ...
  - Governments are generally able to repay the principal and interest, making these bonds nearly risk-free (hence, they are often considered risk-free rates).^[Read: [Default Trap](https://en.wikipedia.org/wiki/Default_trap?utm_source=chatgpt.com)]
- Overnight Rates
...
- Reference interest rates play a crucial role in financial markets.
...
![](../img/interest_libor){fig-align="center"}

Source: [https://www.bis.org/publ/qtrpdf/r_qt2212e.htm](https://www.bis.org/publ/qtrpdf/r_qt2212e.htm)
...
# ---- Fetch U.S. Treasury Yields from FRED ----
us_symbols <- c("DGS1MO", "DGS3MO", "DGS6MO", "DGS1", "DGS2", "DGS3", "DGS5", "DGS7", "DGS10", "DGS20", "DGS30")
getSymbols(us_symbols, src = "FRED")

# Extract the most recent yields
us_yields <- sapply(us_symbols, function(sym) last(get(sym)))
us_maturities <- c(1/12, 3/12, 6/12, 1, 2, 3, 5, 7, 10, 20, 30)  # Convert months to years

# Create U.S. yield curve data frame
us_curve <- data.frame(
  Maturity = us_maturities,
  Yield = as.numeric(us_yields),
  Country = "U.S. Treasury"
)
...
# Select only the required maturities and yield values from Column D
selected_rows <- c("국고채권(1년)", "국고채권(2년)", "국고채권(3년)", "국고채권(5년)", "국고채권(10년)", "국고채권(30년)")

# Filter the data
korea_filtered <- korea_data %>%
  filter(종류명 %in% selected_rows) %>%  # Assuming row names are in the first column
...
yield_curve <- rbind(us_curve, korea_curve)

# ---- Plot the Yield Curves ----
ggplot(yield_curve, aes(x = Maturity, y = Yield, color = Country)) +
  geom_line(size = 1.2) +
  geom_point(size = 2) +
  scale_x_continuous(breaks = unique(yield_curve$Maturity), labels = number_format(accuracy = 0.01), guide = guide_axis(angle = 45)) +  # Two decimal points
...
- Useful for benchmarking and pricing: Can compare bonds with different coupon rates.
